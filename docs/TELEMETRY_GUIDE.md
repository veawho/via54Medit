...
8. **默认排程、法定节假日与「别关机」提醒**：
   ...
   **顺延规则**：目标日落在周末或法定节假日时改到下一个工作日。
   ...
   - **补班的周六算工作日**，不会被误顺延；只按周末判断就会踩这个坑。
   - **提醒日按顺延后的日期算**，两者共用 `schedule.py` 一份实现（`daemon` 与 `reminders` 都调它），因此"提醒说明天推、实际推到假期后"这种自相矛盾不会出现。
   - **相邻两次目标可能落到同一天**：春节整周连休时 02-16 与 02-23 两个周一都顺延到 02-24；提醒侧按「类型 + 生效时刻」去重，守护进程侧靠"当天只发一次"兜住。
   - **只改"哪一天发"，不改"报哪一期"**：报表周期仍按实际发送日计算，卡片 `period_name`、表格行与多维表格记录三者自洽。
   - **提醒文案会跟着变**：提醒日与推送日隔着整段假期时不再说"今晚请不要关机"，改为"请在推送日之前确保设备开机联网"。
   - 关掉顺延：`config --no-defer`。
...
# 6.1 顺延规则 (目标日遇周末/法定节假日时改到下一个工作日, 默认开启)
python -m telemetry.cli config --no-defer    # 关闭顺延, 按固定日期发送
python -m telemetry.cli config --defer       # 重新开启
