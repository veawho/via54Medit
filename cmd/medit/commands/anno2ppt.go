...
var anno2pptOcrCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		scriptPath := filepath.Join(os.Getenv("HOME"), ".hermes", "skills", "via54medit", "via54medit-anno2ppt-phase7", "scripts", "paddleocr_pdf_page.py")
		// 如果没安装到 skill, fallback 到本地
		if _, err := os.Stat(scriptPath); os.IsNotExist(err) {
			scriptPath = filepath.Join(os.Getenv("HOME"), "Desktop", "developments", "via54Medit", "scripts", "paddleocr_pdf_page.py")
		}

		pyExe := filepath.Join(os.Getenv("HOME"), ".hermes", "hermes-agent", "venv", "bin", "python3.11")
