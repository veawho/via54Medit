...
* **核心亮点**：
  ...
  9. **统计数据准确且实时**：只统计**真实产出物**（备份/临时目录里的副本不计入）；已收录条目的可变字段（高亮页数与标注数、下载文件大小）会跟着最新观测值**刷新而不是冻结**；守护进程每 30 秒巡检、报表当场从 SQLite 计算不做缓存，`medit-telemetry refresh` 可立即刷新，`status` 末尾给出"最近入库时间"作为新鲜度凭据。
